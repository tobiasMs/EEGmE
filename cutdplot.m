...
for session=1:8
    ...
    for kata = 1:60
        ...
        if statAll == 1
            ...
            if resp (kata)<0.5
                filename=strcat(fileExcel(1:2),'_W',num2str(kata),'_F.png');
                counterFast=counterFast+1;
            elseif resp (kata)>=0.5
                filename=strcat(fileExcel(1:2),'_W',num2str(kata),'_S.png');
                counterSlow=counterSlow+1;
            end
            %
            figure(1)
            topoplot(Norm_pow, 'eloc16.txt','colormap',flipud(hot), 'electrodes','ptslabels');     % memanggil fungsi topoplot
